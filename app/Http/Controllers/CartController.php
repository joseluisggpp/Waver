...
class CartController extends Controller
{
    ...
    public function index()
    {
        ...
        $price = $this->calcularTotal($products);
        $iva = $price[1];
        $total = $price[0];
        $subtotal = $total - $iva;
    ...
    public function add(Song $song)
    {
        ...
        if ($carrito) {
            ...
            // Si el producto no está en el carrito, crear un nuevo registro con cantidad 1
            if (Order::getProducts()->where('producto_idProducto', $producto->id)->first()) {
                return redirect()->route('top10songs')->with('error', 'La canción ya está en tu carrito.');
            }
            Cart::create([
                'usuarios_idUsuario' => auth()->id(),
    ...
    public function checkout()
    {
        ...
        $order = [
            ...
            "total" => $this->calcularTotal($cartItems)[0],
    ...
    private function calcularTotal($products)
    {
        // Calcular subtotal, IVA y total
